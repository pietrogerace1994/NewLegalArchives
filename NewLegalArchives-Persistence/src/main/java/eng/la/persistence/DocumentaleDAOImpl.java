...
@Component(value="documentaleDAO")
public class DocumentaleDAOImpl implements DocumentaleDAO {
	private String url;
	...
	public DocumentaleDAOImpl(String url, String username, String password, String stanza, String osname) {
		this.url = url;
		this.osname = osname;
	...
	@Override
	public void verificaCreaPercorsoCartellaEmail(String cartellaPadreFascicolo) throws Throwable {
		StringTokenizer tokenizer = new StringTokenizer(cartellaPadreFascicolo, "\\/");
		Folder cartellaPadre = leggiCartella("/");
		String path = "/";
		while( tokenizer.hasMoreTokens() ){
			...
			if( folder == null ){
				String uuid = UUID.randomUUID().toString();
				cartellaPadre = creaCartella(uuid, token, FileNetClassNames.PRESIDIO_NORMATIVO_FOLDER, null, cartellaPadre);
			}else{
				cartellaPadre = folder;
			}
		}
	...
	public Boolean testFilenet() throws Throwable {
		Boolean isOk=true;
		try {
			new FileNetWrapper(url, stanza, osname, username, password);
		} catch (Exception e) {
			isOk =  false;
		}
		return isOk;
