package test;

import java.util.List;

import org.springframework.context.support.ClassPathXmlApplicationContext;

import eng.la.business.EventoService;
import eng.la.model.view.EventoView;
import junit.framework.TestCase;

public class EventoTest extends TestCase {
	ClassPathXmlApplicationContext ctx; 
	
	public ClassPathXmlApplicationContext getCtx() {
		return ctx;
	}

	public void setCtx(ClassPathXmlApplicationContext ctx) {
		this.ctx = ctx;
	}
	
	 
	// 20160701 - Test --
	public void testLeggiEvento() {
		configure(); 
		EventoService service = (EventoService) ctx.getBean("eventoService");
		try { 
			List<EventoView> eventiView = service.leggi();
			System.out.println("Lettura eventiView riuscita: "+ eventiView == null? 0 : eventiView.size());
		 

			assertTrue(true);
		} catch (Throwable e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			assertTrue(false);
		}
	}
	
	private void configure() { 
		if( LATestSuite.getCtx() == null ){
			ctx = new ClassPathXmlApplicationContext(new String[]{ "applicationContext.xml" });
		}else{
			ctx = LATestSuite.getCtx();
		}
	}
}