...
<section id="content">
	<div class="container internal-page">
		<div class="row">
			<div id="col-1"
				class="col-lg-12 col-md-12 col-sm-12 col-sx-12 connectedSortable">
				...
				<div class="card">
					<div class="card-header cw-header palette-Amber bg">
						<h1>
							<img
								src="<%=request.getContextPath()%>/portal-news/portal/img/ico_archivio_header.png">
					...
					<div class="card-body">
						<div class="section ricerca card-padding">
							...
							<div class="col-md-12">
								<div class="form-group fg-line col-md-10">
									<label for="search"></label> <input
										...
										placeholder="Cerca newsletter..." type="text">
								</div>
								<div class="form-group fg-line col-md-2">
									<button type="submit"  onclick="portalenews.filtra_search()" 
										class="btn cerca bgm-black btn-sm m-t-10 waves-effect"><spring:message text="??publicNews.box.label.cerca??" code="publicNews.box.label.cerca" /></button>
								</div>
							</div>
							<div class="col-md-2">
								<div class="form-group fg-line col-md-2">
							...
							<div class="col-md-10">
								<div class="form-group fg-line col-md-12 filtri row">
									<label><spring:message text="??publicNews.box.label.categoria??" code="publicNews.box.label.categoria" /></label>

									<c:if test="${ ltsCategoriaPadre != null }">
										<button type="button" onclick="portalenews.categoria_tutte()" 
											class="btn filtro btn-color0 active btn-sm m-t-10 waves-effect">tutte</button>
										<c:forEach items="${ltsCategoriaPadre}" var="oggetto">

											<button type="button" id="${ oggetto.id }"
												onclick="portalenews.seleziona_categoria(this)"
												class="btn filtro btn-${oggetto.css} btn-sm m-t-10 waves-effect <c:if test="${ idCatPadre eq oggetto.id }"> active </c:if>">
						...
						<input type="hidden" id="newsletterid" value="${newsletterid}">
							<ul>
								<c:if
									test="${ articleListCustom != null && not empty articleListCustom.lstNewsletterArticleCustom}">

									<c:forEach
										items="${articleListCustom.lstNewsletterArticleCustom}"
										var="oggetto">
										<li><a> <img
												...
												<c:forEach items="${oggetto.articolos}" var="article">
													...
													<div
														class="content-news color5 col-md-12 col-lg-12 col-sm-12">
					 			 ...
					 			 <div class="row">
                                             <div class="col-sm-12">
												...
												<c:otherwise>
											       <li class="first" aria-disabled="false"><a data-page="1" class="button pagination-news-article">«</a></li>
                                                   <li class="prev" aria-disabled="false"><a data-page="${paginaSelezionata-1}" class="button pagination-news-article">&lt;</a></li>
												</c:otherwise>
												</c:choose>
												<c:if test="${ articleListCustom != null && not empty  articleListCustom.lstNewsletterArticleCustom }">
                                            	<c:forEach var="npg" begin="1" end="${totalePagina}">
                                            	 	<li class="page-1 <c:if test="${ paginaSelezionata eq npg }"> active</c:if>" aria-disabled="false" aria-selected="true"><a data-page="${npg}" class="button pagination-news-article">${npg}</a></li>
                                            	</c:forEach>
                                            	</c:if>
                                                 <c:choose>
												 ...
												 <c:otherwise>
											    	<li class="next" aria-disabled="false"><a data-page="${paginaSelezionata+1}" class="button pagination-news-article">&gt;</a></li>
                                                	<li class="last" aria-disabled="false"><a data-page="${totalePagina}" class="button pagination-news-article">»</a></li>
												 </c:otherwise>
												 </c:choose>   
											   </ul>
                                        	</div>
					...
					<a href="#" title="torna all'inizio del contenuto"
						class="ScrollTop js-scrollTop js-scrollTo" style="display: block;">
						<span>top</span><img
						src="<%=request.getContextPath()%>/portal-news/portal/img/top.png">
					</a>
				</div>
				<!--/ fine col-1 -->
			</div>
