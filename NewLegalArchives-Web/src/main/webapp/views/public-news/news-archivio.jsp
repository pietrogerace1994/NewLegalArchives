                        ...
                        <div class="card">
                            <div class="card-header cw-header palette-Amber bg">
                                <h1><img src="<%=request.getContextPath()%>/portal-news/portal/img/ico_archivio_header.png">
                                  <spring:message text="??publicNews.box.header.titolo??"
										code="publicNews.box.header.titolo" />
                                 </h1>
                            </div>
                            <div class="card-body">
                                    <div class="section ricerca card-padding">
                                           ...
                                           <div class="col-md-12">
                                              ...
                                              <div class="form-group fg-line col-md-2"> 
                                               <button type="button" onclick="portalenews.filtra_search()" class="btn cerca bgm-black btn-sm m-t-10 waves-effect">
										...
										code="publicNews.box.label.anno" /> </label>
                                            <select name="anno" id="anno" onchange="portalenews.categoria_tutte()">
                                           ...
                                           <div class="form-group fg-line col-md-10 filtri"> 
										...
										code="publicNews.box.label.categoria" /> </label>
											 ...
											 <button type="button" id="${ oggetto.id }" onclick="portalenews.seleziona_categoria(this)" class="btn filtro btn-${oggetto.css} btn-sm m-t-10 waves-effect"><c:out value="${oggetto.nomeCategoria}"></c:out></button>	
												</c:forEach>
											</c:if>
                                    ...
                                    <div class="section2">
                                        <div class="col-md-12">
                                            <ul>
                                            <c:if test="${ newsletterCustom != null && not empty  newsletterCustom.lstNewsletter }">
                                            	<c:forEach items="${newsletterCustom.lstNewsletter}" var="oggetto">
											 
											   <li>
                                                    <a href="javascript:void(0)" onclick="portalenews.open_newsletter('${oggetto.id}')">
                                                        <img src="<%=request.getContextPath()%>/portal-news/portal/img/lista_rassegna.png"><span>
                                                        <c:if test="${not empty oggetto.dataCreazione}">
							                            <fmt:formatDate pattern="dd/MM/yyyy" value="${oggetto.dataCreazione}"/>
							                            </c:if>
                                                    </span> ${oggetto.titolo}
                                                    </a>
                                                </li>
                                ...
                                <div class="row">
                                             <div class="col-sm-12">
												...
												<c:otherwise>
											       <li class="first" aria-disabled="false"><a data-page="1" class="button pagination-archive">«</a></li>
                                                   <li class="prev" aria-disabled="false"><a data-page="${paginaSelezionata-1}" class="button pagination-archive">&lt;</a></li>
												</c:otherwise>
												</c:choose>
												<c:if test="${ newsletterCustom != null && not empty  newsletterCustom }">
                                            	<c:forEach var="npg" begin="1" end="${totalePagina}">
                                            	 	<li class="page-1 <c:if test="${ paginaSelezionata eq npg }"> active</c:if>" aria-disabled="false" aria-selected="true"><a data-page="${npg}" class="button pagination-archive">${npg}</a></li>
                                            	</c:forEach>
                                            	</c:if>
                                                 <c:choose>
												 ...
												 <c:otherwise>
											    	<li class="next" aria-disabled="false"><a data-page="${paginaSelezionata+1}" class="button pagination-archive">&gt;</a></li>
                                                	<li class="last" aria-disabled="false"><a data-page="${totalePagina}" class="button pagination-archive">»</a></li>
												 </c:otherwise>
												 </c:choose>   
											   </ul>
                                        	</div>
