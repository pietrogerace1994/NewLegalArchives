					...
					<div id="col-1" class="col-lg-12 col-md-12 col-sm-12 col-sx-12">
						...
						<div class="card">
							<div class="card-header ch-dark palette-Green-SNAM bg">
									<h2>
										<spring:message text="??nazione.label.gestione??"
											code="nazione.label.gestione" />
									</h2>
								
							</div>
							<div class="card-body">

								<form:form name="nazioneEditForm" method="post"
									...
									<fieldset class="scheduler-border">
    									<legend class="scheduler-border"><spring:message text="??nazione.label.modifica??"
											code="nazione.label.modifica" /></legend>
														...
														<div class="form-group">
															<c:if test="${ nazioneView.listaLingua != null }">
																<c:forEach items="${nazioneView.listaLingua}" var="lin" varStatus="status">
																	...
																	<div class="col-sm-10">
																		<input onfocus="editCheck()" type="text" name="nazioneDesc[${status.index}]" class="form-control"/>
																	</div>
																</c:forEach>
															</c:if>
															<label class="col-sm-2 control-label">
															<spring:message text="??nazione.label.partiCorrelate??"
																code="nazione.label.partiCorrelate" />
															</label>
															<form:checkbox path="soloParteCorrelata" onchange="editCheck()"/>
														</div>
													</div>
												</div>
										</div>
									...
									<fieldset class="scheduler-border">
    									<legend class="scheduler-border"><spring:message text="??nazione.label.inserimento??"
											code="nazione.label.inserimento" /></legend>
										<div class="list-group lg-alt">
											<div class="list-group-item media">
												<div class="media-body">
													<div class="form-group">
														<c:if test="${ nazioneView.listaLingua != null }">
															<c:forEach items="${nazioneView.listaLingua}" var="ling" varStatus="status">
																<label for="nazioneIns[${status.index}]" class="col-sm-2 control-label">${ling.vo.descrizione}</label>
																<div class="col-sm-10">
																	<input onfocus="insertCheck()" type="text" name="nazioneIns[${status.index}]" class="form-control"/>
																</div>
															</c:forEach>
														</c:if>
														<label class="col-sm-2 control-label">
														...
														<form:checkbox path="soloParteCorrelataIns" onchange="insertCheck()"/>
													</div>
												</div>
											</div>
										</div>	
								...
								<button form="nazioneEditForm" onclick="cancellaNazione()"
									class="btn palette-Green-SNAM bg btn-float2 waves-effect waves-circle waves-float">
									<i class="zmdi zmdi-delete"></i>
								</button>
								<button form="nazioneEditForm" onclick="salvaNazione()"
