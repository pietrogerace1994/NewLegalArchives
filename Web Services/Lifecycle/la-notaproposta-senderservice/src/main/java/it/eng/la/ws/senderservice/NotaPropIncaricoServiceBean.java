...
import it.eng.la.ws.entity.NotaPropIncarico;
...
public class NotaPropIncaricoServiceBean implements NotaPropIncaricoSenderService
{
	public static SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy", Locale.ITALY);
	
	@PersistenceContext(unitName="NotaPropIncaricoSender-PU")
	EntityManager emLa;
	...
	protected final static Logger GENERAL_LOGGER = Logger.getLogger(NotaPropIncaricoServiceBean.class.getName());
	
	
	public void sendFile()
	{
		String tipoDoc = "PDFNP";
		...
		if(getSemaforo(tipoDoc))
		{
			...
			{
				tx.begin();
				GENERAL_LOGGER.debug("**** START SEMAFORO VERDE -- INVIO DATI NotaPropIncarico ****");
				
				List<NotaPropIncarico> resultListLI = emLa.createNamedQuery("NotaPropIncarico.findInviata").
					...
					if(resultListLI!=null && resultListLI.size()>0)
					{
						...
						for (NotaPropIncarico lt : resultListLI) 
						{							
							...
							try
							{
								Esito invokeFillPDFLifeCycle = fileWSPort.invokeFillPDFLifeCycle("xdpNotaPropostaIncaricoPDF", bytes);
								...
								else
								{
									...
									risultatoEsito.setInviata(1); 
									risultatoEsito.setFileName("NotaPropostaIncarico"+lt.getId()+".pdf");
								}
								GENERAL_LOGGER.debug("NotaPropostaIncarico "+lt.getId()+" Flag Inviata : "+risultatoEsito.getInviata());
							}
							catch(Exception e)
							{
			...
			catch(Exception e)
			{
				...
				for (Iterator iterator = list.iterator(); iterator.hasNext();) 
				{
		        	...
		        	{ 
		        		...
		        		GENERAL_LOGGER.debug("NotaPropostaIncarico "+esitoList1.getStateid()+" Flag Inviata : "+esitoList1.getInviata());
		        	}
				}
			...
			finally
			{
				try 
				{
					...
					{
						tx.begin();
						//indipendetemente da come va l'invocazione del servizio, devo aggiornare i record in NotaPropostaIncarico
						for(Iterator iterator = list.iterator(); iterator.hasNext();) 
						{
							RisultatoEsito esitoList = (RisultatoEsito) iterator.next();
							List<NotaPropIncarico> existIdList = emLa.createNamedQuery("NotaPropIncarico.byid").
									setHint("javax.persistence.cache.retrieveMode", "BYPASS").
									setParameter("id", esitoList.getStateid()).getResultList();						
							GENERAL_LOGGER.debug("START update in NotaPropostaIncarico");
							if(existIdList!=null && existIdList.size()>0)
							{
								for (NotaPropIncarico existf : existIdList) 
								{
									existf.setInviata(esitoList.getInviata());
									existf.setDataInvio(new Date());
									GENERAL_LOGGER.debug("Aggiornamento in NotaPropostaIncarico dello Stateid: "+esitoList.getStateid() +" - Inviata: "+esitoList.getInviata());
					...
					GENERAL_LOGGER.debug("**** END SEMAFORO VERDE -- INVIO DATI NotaPropIncarico ****");
				} 
				catch (Exception e) 
				{
					GENERAL_LOGGER.error("Errore durante il salvataggio in NotaPropostaIncarico ",e);
					try 
					{
						tx.rollback();
					} 
		...
		{
			GENERAL_LOGGER.debug("**** SEMAFORO ROSSO -- INVIO DATI NotaPropIncarico ****");
		}
	}
	  ...
	  //@TransactionAttribute(TransactionAttributeType.REQUIRED) 
		public void init() 
		{
			...
			for (Object obj : timerService.getTimers())
			{
				...
				if(scheduled.equalsIgnoreCase("NotaPropTimer"))
				{
					timer.cancel();
			...
			timerService.createTimer((long)(INITIAL_DURATION),(long)(INTERVAL_DURATION), "NotaPropTimer");
			GENERAL_LOGGER.debug("Nuovo Timer NotaPropTimer creato ogni "+INTERVAL_DURATION+" millisecondi!");
		}
