...
import it.eng.la.ws.entity.LetteraIncarico;
...
public class FileServiceBean implements FileSenderService
{
	public static SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy", Locale.ITALY);
	
	@PersistenceContext(unitName="FileSender-PU")
	EntityManager emLa;
	...
	protected final static Logger GENERAL_LOGGER = Logger.getLogger(FileServiceBean.class.getName());
	
	
	public void sendFile()
	{
		String tipoDoc = "PDFLI";
		LetteraIncarico li=new LetteraIncarico();
		...
		if(getSemaforo(tipoDoc))
		{
			...
			{
				tx.begin();
				GENERAL_LOGGER.debug("**** START SEMAFORO VERDE -- INVIO DATI FILE ****");
				
				List<LetteraIncarico> resultListLI = emLa.createNamedQuery("LetteraIncarico.findInviata").
					...
					if(resultListLI!=null && resultListLI.size()>0)
					{
						...
						for (LetteraIncarico lt : resultListLI) 
						{							
							...
							try
							{
								Esito invokeFillPDFLifeCycle = fileWSPort.invokeFillPDFLifeCycle("xdpLetteraIncaricoPDF", bytes);
								...
								else
								{
									...
									risultatoEsito.setInviata(1); 
									risultatoEsito.setFileName("LetteraIncarico"+lt.getId()+".pdf");
								}
								GENERAL_LOGGER.debug("LetteraIncarico "+lt.getId()+" Flag Inviata : "+risultatoEsito.getInviata());
							}
							catch(Exception e)
							{
			...
			catch(Exception e)
			{
				...
				for (Iterator iterator = list.iterator(); iterator.hasNext();) 
				{
		        	...
		        	{ 
		        		...
		        		GENERAL_LOGGER.debug("LetteraIncarico "+esitoList1.getStateid()+" Flag Inviata : "+esitoList1.getInviata());
		        	}
				}
				...
				try 
				{
					if(trovato)
					{
						...
						for(Iterator iterator = list.iterator(); iterator.hasNext();) 
						{
							...
							List<LetteraIncarico> existIdList = emLa.createNamedQuery("LetteraIncarico.byid").
									setHint("javax.persistence.cache.retrieveMode", "BYPASS").
									setParameter("id", esitoList.getStateid()).getResultList();		
							GENERAL_LOGGER.debug("START update in LetteraIncarico");
							if(existIdList!=null && existIdList.size()>0)
							{
								for (LetteraIncarico existf : existIdList) 
								{
									existf.setInviata(esitoList.getInviata());
									existf.setDataInvio(new Date());
									GENERAL_LOGGER.debug("Aggiornamento in lettera incarico dello Stateid: "+esitoList.getStateid() +" - Inviata: "+esitoList.getInviata());
					...
					GENERAL_LOGGER.debug("**** END SEMAFORO VERDE -- INVIO DATI File ****");
				} 
				catch (Exception e) 
				{
					GENERAL_LOGGER.error("Errore durante il salvataggio in Lettera di incarico ",e);
					try 
					{
						tx.rollback();
					} 
		...
		{
			GENERAL_LOGGER.debug("**** SEMAFORO ROSSO -- INVIO DATI FILE ****");
		}
	}
	  ...
	  //@TransactionAttribute(TransactionAttributeType.REQUIRED) 
		public void init() 
		{
			...
			for (Object obj : timerService.getTimers())
			{
				...
				if(scheduled.equalsIgnoreCase("FileTimer"))
				{
					timer.cancel();
			...
			timerService.createTimer((long)(INITIAL_DURATION),(long)(INTERVAL_DURATION), "FileTimer");
			GENERAL_LOGGER.debug("Nuovo Timer FileTimer creato ogni "+INTERVAL_DURATION+" millisecondi!");
		}
